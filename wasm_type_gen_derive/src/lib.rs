...
/// ```
/// pub struct Hello;
/// use wasm_type_gen_derive::output_and_stringify_basic;
/// #[output_and_stringify_basic(myimplstr)]
/// impl Hello {
///     pub fn hello() {}
/// }
/// // the entire Hello impl will be stringified into `myimplstr`
/// 
/// println!("{myimplstr}");
/// ```
#[proc_macro_attribute]
pub fn output_and_stringify_basic(attr: proc_macro::TokenStream, item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let attr = proc_macro2::TokenStream::from(attr);
    let mut attr_iter = attr.into_iter();
    let id = attr_iter.next().expect("Must provide an attribute to be the name of the variable that the string is output to");
    let var_to_put_string = if let proc_macro2::TokenTree::Ident(id) = id {
